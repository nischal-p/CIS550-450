...
export default class Dashboard extends React.Component {
  ...
  showMovies(keyword) {
    ...
    fetch(`http://localhost:8081/keywords/${keyword}`,
    {
      ...
      // create movies (list of dashboard rows)
      const movies = moviesList.map((movieObj, i) => 
        <DashboardMovieRow 
        title={movieObj.title}
        key={i}
        rating={movieObj.rating}
        numRatings={movieObj.num_ratings}
        />
      );

      // Set the state of the movies to the value returned by the HTTP response from the server.
      this.setState({
        movies: movies
      });
    }, err => {
